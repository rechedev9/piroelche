...
- **Medios:** incorporar archivos autorizados a `public/media/` y referenciarlos con rutas locales seguras. El bloque `brand` contiene las imágenes editoriales compartidas; `fit: "contain"` conserva envases completos. Imágenes: dimensiones y texto alternativo. Vídeo: poster, descripción, `audio` explícito y pista VTT si contiene sonido. Registrar permisos y correspondencia con el artículo/caso. Sin material se conserva el espacio con un placeholder neutral.
- **Lector del catálogo:** `public/media/catalogo-2026/manifest.json` conserva orden, páginas, miniaturas, texto extraído y hashes; `src/lib/catalogue.ts` define los enlaces de familia a páginas originales. Una edición nueva exige regenerar y revisar todas sus páginas y ajustar ese mapa. `tests/catalogue.test.ts` verifica archivos, dimensiones, hashes y navegación. Los precios vacíos o textos incompletos del original se conservan, sin rellenarlos por inferencia.
