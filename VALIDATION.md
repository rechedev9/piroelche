...
Fecha: 9 de septiembre de 2026. Entorno local Windows 11 25H2, Node 24.18.0, pnpm 11.22.0, Next 16.3.4, TypeScript 5.9.3, Tailwind 4.3.3 y shadcn CLI 4.21.0. Build productivo local actual: `9mOHWM-Tp-bJEqmVhnydW`. No se ha hecho push, despliegue, cambio de producción ni envío a personas.

## Ajustes de frontend tras el análisis crítico

Fecha: 10 de septiembre de 2026. Partiendo del commit `ba9309b`, se aplicaron los ajustes P1/P2/P3 de un análisis crítico del frontend contrastado con las normas del repositorio Berrus. Candidato final: árbol Git `b9e67b56428e9d43ed4d75203941d563c51fdf4e` (24 archivos modificados y 7 nuevos; sin cambios en `content/site.json`, fixtures, PDF, rutas, contrato de formularios ni textos legales). Cambios: tokens shadcn cableados a la marca en `@theme inline`; colores duros repetidos sustituidos por tokens con valores resueltos idénticos, salvo un gris del catálogo (`#dddbe1`) unificado con `#dedbe0`, a una unidad de diferencia; eliminación de la cascada de `!important` en proporciones de medios (solo queda el bloque de movimiento reducido); fuentes servidas con `next/font/local` desde los woff2 latinos ya instalados, con métricas de respaldo; fondo decorativo del inicio en WebP (45,8 KB frente a 1,47 MB; el PNG original se conserva sin referencias); prioridad de carga del héroe; formulario dividido en `lib/lead-draft.ts`, `hooks/use-lead-draft.ts` y componentes de campos y éxito sin cambiar el DOM ni el contrato; etiqueta, control y error como hermanos para que el nombre accesible no repita el error; composición de `onClick` en enlaces trazados; JSON-LD de migas centralizado; dirección de LocalBusiness como PostalAddress y horario derivado del modelo existente; enlace a solicitud de evento centralizado; pie en su propio componente; CSS muerto retirado; áreas de pulsación de pie y migas a 40 px. Esta última es la única diferencia visible: el pie crece unos 8 px por enlace y la fila de migas unos 5 px; anchos, tipografía y colores no cambian.

Comprobaciones ejecutadas hoy sobre el candidato final: **lint, typecheck y build correctos; 50/50 pruebas unitarias/integración; 96/96 E2E** en Chromium, Firefox y WebKit, sin reintentos ni inestables ([resultados](evidence/frontend-refresh/e2e-results.json)). La matriz intermedia, antes de la migración de fuentes y de restaurar la carga anticipada del fondo, también dio 96/96. Recorrido con Playwright de seis rutas a 360, 1024 y 1440 px: sin desbordamiento horizontal, también con una palabra de 300 caracteres inyectada en H1 y párrafo; menú móvil abre, cierra con Escape y devuelve el foco; en la demo, el campo con error expone el nombre «Tu nombre» sin repetir el mensaje y conserva `aria-describedby`. Una compilación independiente de las tres hojas con Tailwind 4.3.3, antes y después, mostró únicamente las diferencias previstas.

Lighthouse móvil local, mismo método antes y después ([base](evidence/frontend-refresh/performance/baseline-ba9309b-mobile.txt), [final](evidence/frontend-refresh/performance/final/)), medido tras la matriz E2E definitiva: Inicio 95 → 92/94/94 en tres muestras (LCP 3,0 s → 3,3/3,1/3,1 s); Eventos 89 → 92 (LCP 3,7 → 3,4 s); Catálogo 94 → 95 (LCP 3,1 → 3,0 s). CSS bloqueante estimado: 360 ms → 90–100 ms. CLS 0 en todas. Accesibilidad y buenas prácticas 100; SEO 69 por el noindex intencional. Inicio queda en paridad dentro del ruido de laboratorio (±0,3 s entre muestras consecutivas); una versión intermedia sin carga anticipada del fondo del héroe, que en móvil es el elemento LCP, medía 93 y 3,2–3,3 s de forma consistente y se corrigió antes del candidato final; esos JSON intermedios no se conservan. Las URL base de Eventos y Catálogo se midieron sin barra final y sufrían una redirección de unos 600 ms que la medición final no incluye, así que la mejora de esas dos páginas está parcialmente sobrestimada. Son muestras de laboratorio en localhost, no datos de campo.

Decisión documentada: la lámina del catálogo sigue sirviéndose sin optimizar (unos 880 KB por página). El agente de rendimiento la pasó por el optimizador, pero seis pruebas E2E exigen que el escaneo se entregue a resolución original, un contrato deliberado del lector; se revirtió en vez de relajar la prueba. Optimizarla supone un cambio de contrato pendiente de decisión. Quedan fuera de esta etapa, por depender de decisiones externas: migración completa del CSS a utilidades, unificación de diálogos, caché por ruta en lugar de `force-dynamic` global, imagen OG de 1200×630, retirada del PNG sin referencias y del PDF de 82 MB del repositorio. Novedades de configuración: `images.formats` con AVIF primero y `images.qualities` limitado a 60 y 75.

Dos revisiones independientes estáticas **P0 únicamente**, por agentes aislados distintos, no identificaron P0. [Informe](docs/FRONTEND_REVIEW.md). La primera se hizo mientras el fondo del héroe cambiaba una vez y releyó la versión final; la segunda, sobre el árbol de código congelado, verificó por blob al inicio y al final que los 30 archivos de código, configuración, CSS, pruebas y activos no derivaron, y ejecutó por su cuenta lint, typecheck y las 50 pruebas unitarias. Tras las revisiones solo se completaron documentos y evidencias. No se usó autoreview. Las observaciones fuera de alcance quedan registradas en el informe. El resultado no acredita otras prioridades ni recepción real del negocio; publicar sigue pendiente de textos legales aprobados, destino autorizado para formularios y autorización.

## Actualización completa de marca y catálogo

El usuario amplió el encargo para sustituir todas las imágenes pertinentes, incorporar bien el catálogo y dar al frontend más carácter de empresa de pirotecnia. Las seis secciones públicas usan fotografías reales del negocio y recortes fieles del PDF. Se mantienen Tailwind, shadcn, los colores de marca, rutas y formularios. El catálogo completo tiene 16 páginas, índice, enlaces por familia, navegación recargable, ampliación Radix y texto extraído legible sin JavaScript. El original de 81,9 MB permanece intacto y solo se obtiene por acción del visitante. Las fixtures no se publican como inventario ni casos.

Comprobaciones finales: **lint, typecheck y build correctos; 50/50 pruebas unitarias/integración; 96/96 E2E** en Chromium, Firefox y WebKit, sin reintentos, omisiones ni pruebas inestables. [Resultados E2E](evidence/brand-refresh/e2e-results.json), [unitarias](evidence/brand-refresh/unit.log) y [build](evidence/brand-refresh/build.log). La primera matriz obtuvo 87/96: nueve expectativas conservaban el H1 anterior «Catálogo»; se actualizó al nuevo título y se repitió la matriz completa. No se relajó la validación.

Las pruebas recorren las 16 páginas y comprueban también navegación y lectura sin JavaScript; verifican que el PDF no se descarga automáticamente, abren Fuego frío en su página original, amplían, desplazan con teclado y recuperan el foco al cerrar. Axe verifica las secciones públicas y demo y el diálogo abierto. Se comprobaron 21 combinaciones de siete rutas a **390, 1366 y 1920 px**, con fotos decodificadas, cero placeholders públicos, títulos largos sin espacios y ningún desbordamiento horizontal o error JavaScript; además se capturó el zoom en los tres tamaños. [Capturas y observaciones](evidence/brand-refresh/visual/checks.json). Se inspeccionaron visualmente Inicio, Eventos, Sobre nosotros, Tiendas, Contacto, Catálogo y Tracas. La revisión visual detectó texto blanco sobre el fondo blanco del botón PDF; se corrigió el fondo a transparente y se verificó la captura final. En Brave se recorrieron Inicio → Catálogo → Tracas → Ampliación, y quedaron abiertas las páginas actuales del puerto 3001.

Se revisaron las 16 páginas originales y se verificaron **39 WebP** con sus dimensiones/hashes; las seis fotografías nativas extraídas conservan alfa y píxeles visibles originales. Los textos derivados mantienen finales de línea LF también al obtener el repositorio en Windows, para preservar sus hashes. [Verificación del catálogo](evidence/catalogo-2026/verification.json). Su contenido impreso se conserva; no se completan precios vacíos, descripciones ausentes ni condiciones por inferencia.

Lighthouse local del build final, ejecutado después de la matriz E2E: Inicio y Catálogo obtienen **99/100 en rendimiento móvil, 100/100 en escritorio y 100/100 en accesibilidad automática**. LCP móvil: 1,95 s en ambas páginas; escritorio: 0,58 s y 0,73 s. CLS: 0–0,033. [Informes y entorno](evidence/brand-refresh/performance/summary.json). Son cuatro mediciones de laboratorio sobre localhost, no datos de campo ni INP/p75. El noindex de revisión sigue activo; SEO queda en 69/100 y no se interpreta como rendimiento de una web publicada.

La revisión independiente estática **P0 únicamente** del árbol `266557461592b864f3388fbbd8dc70f808bce92c` no identificó P0. [Informe](docs/BRAND_REVIEW.md). El revisor comprobó 60 archivos mediante hashes; código, contenido, assets y pruebas no cambiaron después de ese candidato. Los documentos y evidencias finales se incorporan al cierre. No se usó autoreview por la exclusión expresa vigente. Este resultado no acredita ausencia de problemas de otras prioridades ni recepción real del negocio: los formularios públicos siguen sin receptor autorizado y se ofrece teléfono como alternativa.

Las secciones siguientes conservan la evidencia histórica de las etapas anteriores; sus builds, resultados y comparaciones geométricas no sustituyen los de esta actualización.
