...
La ampliación visual solicitada por el usuario añade `brand.css` y `catalogue.css`, ambos con `@reference "./globals.css"` y utilidades Tailwind. Conserva los tokens de marca y los componentes shadcn, y cambia las composiciones para dar protagonismo a fotografía real, productos y celebraciones. El visor usa Radix Dialog, enlaces reales sin JavaScript y controles de ampliación con foco y teclado. Estas capturas nuevas sustituyen la comparación geométrica con el prototipo para las secciones rediseñadas; la comparación anterior permanece como evidencia histórica de la migración inicial.
